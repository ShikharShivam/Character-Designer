...
import { User, Flag, Shield, Palette, Sword, History, Box, Activity, Layers, Flame } from 'lucide-react';
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {character.alternateOutfits.map((outfit, idx) => (
                  <div key={idx} className="bg-slate-800/20 p-4 rounded-lg border border-slate-700/60 hover:border-slate-600 transition-colors">
                    <h4 className="text-white font-medium mb-1 text-sm">{outfit.name}</h4>
                    <p className="text-slate-400 text-xs mb-3 min-h-[40px]">{outfit.description}</p>
                    <div className="flex flex-wrap gap-1.5">
                      {outfit.materials.slice(0, 3).map((m, i) => (
                        <span key={i} className="bg-slate-900 text-slate-500 text-[10px] px-1.5 py-0.5 rounded border border-slate-800">
                          {m}
                        </span>
                      ))}
                      {outfit.materials.length > 3 && (
                        <span className="text-[10px] text-slate-600 px-1">+{outfit.materials.length - 3}</span>
                      )}
                    </div>
                  </div>
                ))}
