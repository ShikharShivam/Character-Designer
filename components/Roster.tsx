  ...
  return (
    <div className="mt-24 animate-in fade-in duration-1000 border-t border-master-gold/10 pt-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        {characters.map((char) => {
          if (!char) return null; // Safe guard against null entries
          return (
            <div 
            ...
            >
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                  <p className="text-master-ivory font-serif font-bold text-sm truncate tracking-wide">{char.name || 'Unnamed'}</p>
                  <p className="text-[10px] text-master-gold uppercase tracking-widest truncate mt-1">{char.martialArtStyle || 'Unknown Style'}</p>
                </div>
